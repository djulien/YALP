...
{
//TBD
    duration: 2500/100, //msec; == 50 fr @20 FPS
};
debug({seq});
...
debug("shm", {shmbuf_bytelen: shmbuf.byteLength, NUM_WKERS, wker_shmlen, NUM_PORTS, port_shmlen});
...
const wkstate = ary(NUM_WKERS + 1, wker => new Int32Array(shmbuf, wker * wker_shmlen, u32ofs(wker_shmlen))); //main is 0, wkers are 1..n; At.notify() doesn't like uint32 so use int32
const fbstate = ary_wrap(wkstate[0], "FRTIME_NUMRD, frtime, numrd, X, numwr, numfr, ENDIAN, first32, second32", "WANT_ATOMIC"); //needs to be atomic for multiple threads to access safely
...
const nodes1D = new Uint32Array(shmbuf, (NUM_WKERS + 1) * wker_shmlen); //in-memory copy of layout/model nodes, starts after worker state
const nodes2D = ary(NUM_PORTS, port => new Uint32Array(nodes1D.buffer, port * L2pad(u32len(UNIV_LEN)), UNIV_LEN));
debug({nodes1D_len: u32ofs(nodes1D.byteLength), NUM_PORTS, UNIV_LEN, univlen_L2pad: L2pad(u32len(UNIV_LEN))});
...
//    await startup();
    ...
    fbstate.ENDIAN = NDN_TEST; //also used by workers
    const isBE = (u32(fbstate.first32) == NDN_HI && u32(fbstate.second32) == NDN_LO); //CAUTION: array is int32 (required by Atomics); must use u32 to get correct result
    const isLE = (u32(fbstate.first32) == NDN_LO && u32(fbstate.second32) == NDN_HI);
//    debug(fbstate.first32, fbstate.second32, NDN_HI, NDN_LO); //signed gives wrong result
//    debug(u32(fbstate.first32), u32(fbstate.second32), u32(NDN_HI), u32(NDN_LO));
//    debug(hex(fbstate.first32), hex(fbstate.second32), hex(NDN_HI), hex(NDN_LO));
//    debug(isBE, fbstate.first32 == NDN_HI, fbstate.second32 == NDN_LO);
//    debug(isLE, fbstate.first32 == NDN_LO, fbstate.second32 == NDN_HI);
    debug({isLE, isBE, first32: hex(fbstate.first32), last32: hex(fbstate.last32), NDN_HI: hex(NDN_HI), NDN_LO: hex(NDN_LO)});
    if (isBE == isLE) throw `endian test broken: isLE/isBE ${isLE}/${isBE}, first/second ${hex(fbstate.first32)}/${hex(fbstate.second32)}, hi/lo ${hex(NDN_HI)}/${hex(NDN_LO)}`.brightRed;
    fbstate.numrd_frtime = fbstate.numwr = 0; //allow wkers to start rendering immediately - gives them a head start
    await cre_wkers(worker, {shmbuf}, NUM_WKERS);
...
//        pivot(nodes1D, pxbuf);
//    }
    await fb.bkgloop(fbstate, nodes1D, seq.duration); //pivot + sync in bkg until eof
//    await quit();
    await Promise.all(cre_wkers.all || []); // || [work()]); //wait for wkers to finish; run on fg if !wkers
    debug("main done".brightMagenta);
}


my_export(worker);
function worker(shdata = {})
{
    let now = Date.now();
    ...
    const isBE = (u32(fbstate.first32) == NDN_HI); //&& fbstate.last32 == NDN_LO);
    for (;;)
    {
...
debug("fbstate", wkstate[0].slice(0, 5)); //[frtime, numrd, FRTIME_NUMRD, numwr, numfr, test]
        const combo = fbstate.FRTIME_NUMRD_bump(), [frtime, numrd] = u64split(combo, !isBE); //[Number(combo >> 32n), Number(combo & (1n << 32n - 1n))]; //split atomic value after read
debug("fbstate", wkstate[0].slice(0, 5)); //[frtime, numrd, FRTIME_NUMRD, numwr, numfr, test]
        debug({combo, isBE, frtime, numrd, frtime_sec: milli(frtime), elapsed: milli(elapsed())});
        if (frtime >= seq.duration) break;
        if (numrd < NUM_PORTS)
        {
            debug("wker work", numrd);
            ++mystate.count;
            now = Date.now();
...
debug("fbstate", wkstate[0].slice(0, 5)); //[frtime, numrd, FRTIME_NUMRD, numwr, numfr, test]
        ...
        debug("wker sleep", mystate.sleep);
        fbstate.frtime_sleep(frtime); //sleep until next frame requested
        debug("wker wake", mystate.sleep);
    }
    now = Date.now();
    mystate.wait += chkpt + now;
    mystate.total += now;
    debug("wker done".brightMagenta);
    debug("stats", {total_sec: milli(mystate.total), num_sleep: mystate.sleep, num_render: mystate.count, wait_sec: milli(mystate.wait), avg_wait: milli(mystate.wait / mystate.count), busy_sec: milli(mystate.busy), avg_busy: milli(mystate.busy / mystate.count)}); //{stats: mystate});
}


function cre_wkers(entpt, shdata = {})
{
...
//        worker.unref();
    ...
    (cre_wkers.all || (cre_wkers.all = [])).push(retval); //allow caller to wait for all workers to finish
    return retval;
