...
//check timer accuracy:
        ...
        usleep(100e3);
        debug("calibration: sleep(100 msec) => %'d usec", now_usec() - started);
...
debug("FBPixels::ctor: px@ 0x%p, bpp %d, size %'lu bytes (info says %'d), rowlen32(w) %'lu, h %'lu, #px %'lu, pxrow[0]@ 0x%p, pxrow[1]@ 0x%p, pxeof[h %'lu]@ 0x%p", m_px, bpp(),  m_height * m_rowlen32 * sizeof(m_px[0]), scrf.smem_len, m_rowlen32, m_height, m_numpx, &pixels[0], &pixels.at(1), m_height, &pixels[m_height]);
...
//    const T* const m_first;
    const T* m_nextgap; //start of next gap
    const int m_rowlen, m_gaplen;
public: //ctor/dtor
    GapPtr(T* ptr, int rowlen, int gaplen): m_ptr(ptr), /*m_first(ptr),*/ m_nextgap(ptr + rowlen), m_rowlen(rowlen), m_gaplen(gaplen) {}
    ~GapPtr() {}
    GapPtr(const GapPtr& that): m_ptr(m_ptr), m_nextgap(m_nextgap), m_rowlen(m_rowlen), m_gaplen(m_gaplen) { *this = that; } //avoid [-Weffc++] warning
    GapPtr& operator=(const GapPtr& that) { new (this) GapPtr(that); return *this; } //avoid [-Weffc++]
public: //methods
    inline bool isgap() const { return m_ptr == m_nextgap; } //(m_ptr - m_first) % m_virtlen < m_reallen; }
public: //operators
//    DisjointPtr& operator++() { DisjointPtr copy = *this; ++m_ptr; return copy; } //pre-inc
    GapPtr/*&*/ operator++(int) { if (++m_ptr == m_nextgap) m_nextgap = m_rowlen + (m_ptr -= m_gaplen); return *this; } //post-inc
    GapPtr& operator--() { --m_ptr; return *this; } //pre-dec
//    DisjointPtr operator--(int) { DisjointPtr copy = *this; --m_ptr; return copy; }; //post-dec
...
//        return *m_ptr;
//    }
//    size_t operator-(const T* rhs) const { return m_ptr - rhs; }
};
#if 1 //unit test
int main()
{
    int ary[3][4] = {0};
    auto dump_ary = [ary]() -> void
    {
        char buf[10 * sizeof(ary) / sizeof(ary[0][0]) + 2] = "";
        for (int i = 0; i < sizeof(ary) / sizeof(ary[0][0]); ++i)
            snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), ", 0x%x", ary[0][i]);
debug("ary: %s", buf + 2);
    };
dump_ary();
    GapPtr<int> ptr(&ary[0][0], 4, 2);
    for (int i = 0; i < 3 * 6; ++i) *ptr++ = 100 + i;
dump_ary();
debug("ary@ 0x%p, ptr end@ 0x%p == ary[%'lu]", &ary[0][0], (int*)ptr, ptr - &ary[0][0]);
    return 0;
}
...
//    typedef size_t (Pivot24::*rowlen_init_t)(void);
    ...
    typedef /*static*/ void (*pxrender_t)(int&, int, FBPixels::color_t*&, data_t[NUMCH]); //SIZEOF(bitmasks)])
...
//NOTE: must be set before using pixels.at()
        ...
        debug("Pivot24 ctor: alloc %'lu bytes/channel = %'lu bytes total @0x%p, #skipcols %'d, isopen? %d", m_chqbytes * sizeof(data_t), NUMCH * m_chqbytes * sizeof(data_t), m_chdata, m_skipcols, isOpen());
    }
...
debug("fill(0x%x) %'lu qbytes took %'d usec (excl refresh), special case? %d", bits, NUMCH * m_chqbytes, now_usec() - started, allbytes);
        dirty(true);
    }
#ifdef USING_NAPI
    Napi::Value fill_method(const Napi::CallbackInfo& info)
...
//    virtual /*inline*/ void pxrender(int x, FBPixels::color_t*& px24ptr, data_t chqbits[SIZEOF(bitmasks)])
    /*virtual*/ static /*inline*/ void pxrender(int& want_debug, int xy, FBPixels::color_t*& px24ptr, data_t chqbits[NUMCH]) //SIZEOF(bitmasks)])
    {
...
//        int svcount = count++, want_debug = m_debug_pivot? m_debug_pivot--: 0; //ensure upd before return
        ...
        if (wanted_debug) debug("pivot[%'d], want debug? %d, chqbytes %'lu vs scr res %'lu px => limit %'lu, #px out limit %'lu = min(chbits %'lu, res %'lu)", count, m_debug_pivot + 1, m_chqbytes, chqbytes_max, chqbytes_limit, pmy.numpx(), chbits(), height() * width());
int err_count = 0;
        for (int xy = 0; xy < chqbytes_limit; ++xy) //fill L2R, T2B
        {
...
//                RETURN(clear_error(), errmsg("pivot loop[%'d/%'d] bad: bp24@ 0x%p vs. pixels@ 0x%p..0x%p", xy, m_chqbytes, pmy.bp24, pmy.firstpx, pmy.lastpx)); //&pixels[0][m_chqbytes], ptr_count));
                clear_error();
                errmsg("pivot loop[xy %'d/%'d] bad: bp24@ 0x%p vs. pixels@ 0x%p..0x%p", xy, m_chqbytes, pmy.bp24, pmy.firstpx, pmy.lastpx); //&pixels[0][m_chqbytes], ptr_count));
                break;
            }
...
//            int newcol = pmy.col + pmy.bp24 - oldbp24; //bp24 - &pixels[0][0]; //(bp24 - &pixels[0][0]) % width();
            ...
            if (!xy) //re-init with correct limit now that expected size is known
            {
...
//here we just check the RGB bits for 0/non-0
            while (oldbp24 < pmy.bp24) //check new px values that were rendered
            {
                ...
                if (RGBbits(*oldbp24))
                    if ((m_pxrender == &pxrender) || (*oldbp24 != ::WHITE) || ((pmy.bp24 - oldbp24) % 3)) //kludge: don't report WS281x start bits; CAUTION: bp24 hasn't been wrapped yet, so apply %3 from bp24 back towards oldbp24
                        if (m_debug_pivot-- > 0) debug("pivot[%'d]: qb[%'d/%'lu]/px[%'lu/%'lu] = 0x%x, wh? %d, rerow %lu", count, xy, m_chqbytes, oldbp24 - pmy.firstpx, pmy.numpx(), *oldbp24, *oldbp24 != ::WHITE, rerow(oldbp24 - pmy.firstpx, scrv.xres, scrv.xtotal()));
...
//if (count < 3) debug("pivot[%'d]: px@ %p + %'lu qbytes => pxe@ %p (=+%'lu), %'lu msec", count, &pixels[0][0], m_chqbytes, bp24, bp24 - &pixels[0][0], elapsed);
        if (wanted_debug) //m_debug_pivot-- > 0)
            debug("pivot[%'d]: px@ 0x%p + %'lu qbytes => ptr@ 0x%p (=+%'lu), %'lu trailing px, %'lu/%'lu shadow px (%s bounds), #non-0s %'d, %'u usec", count, pmy.firstpx, m_chqbytes, pmy.bp24, pmy.atpx(), pmy.numpx() - pmy.atpx(), shad.atpx(), shad.numpx(), shad.inbounds()? "in": "out of", non0s, now_usec() - started);
...
//private: //helpers
//    virtual /*inline*/ void pxrender(int x, FBPixels::color_t*& px24ptr, data_t chqbits[SIZEOF(bitmasks)])
    /*virtual*/ static /*inline*/ void pxrender_3ppb(int& want_debug, int xy, FBPixels::color_t*& px24ptr, wsnode_t wsnodes[NUMCH]) //SIZEOF(bitmasks)])
    {
//        static constexpr int count = 0; //TODO: pass in?
//        int& m_ppb = static_m_ppb(); //kludge: create l-value
//        int& m_debug_pivot = static_m_debug_pivot(); //kludge: create l-value
//        if (want_debug) //show channel bits to pivot
        ...
        {
            wsnodes[ch] = LIMIT(wsnodes[ch]); //limit brightness
...
//CAUTION: a lot of memory accesses here; slow on RPi
            color_t px24 = //Abits(::WHITE) | //0xFF000000 |
                ((wsnodes[0] & bit)? bitmasks[0]: 0) |
                ((wsnodes[1] & bit)? bitmasks[1]: 0) |
                ((wsnodes[2] & bit)? bitmasks[2]: 0) |
                ((wsnodes[3] & bit)? bitmasks[3]: 0) |
                ((wsnodes[4] & bit)? bitmasks[4]: 0) |
                ((wsnodes[5] & bit)? bitmasks[5]: 0) |
                ((wsnodes[6] & bit)? bitmasks[6]: 0) |
                ((wsnodes[7] & bit)? bitmasks[7]: 0) |

                ((wsnodes[8] & bit)? bitmasks[8]: 0) |
                ((wsnodes[9] & bit)? bitmasks[9]: 0) |
                ((wsnodes[10] & bit)? bitmasks[10]: 0) |
                ((wsnodes[11] & bit)? bitmasks[11]: 0) |
                ((wsnodes[12] & bit)? bitmasks[12]: 0) |
                ((wsnodes[13] & bit)? bitmasks[13]: 0) |
                ((wsnodes[14] & bit)? bitmasks[14]: 0) |
                ((wsnodes[15] & bit)? bitmasks[15]: 0) |

                ((wsnodes[16] & bit)? bitmasks[16]: 0) |
                ((wsnodes[17] & bit)? bitmasks[17]: 0) |
                ((wsnodes[18] & bit)? bitmasks[18]: 0) |
                ((wsnodes[19] & bit)? bitmasks[19]: 0) |
                ((wsnodes[20] & bit)? bitmasks[20]: 0) |
                ((wsnodes[21] & bit)? bitmasks[21]: 0) |
                ((wsnodes[22] & bit)? bitmasks[22]: 0) |
                ((wsnodes[23] & bit)? bitmasks[23]: 0);
//if ((count < 3) && new24) debug("^%p ('%'lu) <- 0x%x", bp24, )
#if 1
//            if ((px24ptr < &pixels[0][0]) || (px24ptr >= &pixels[height()][0]))
...
//                case 3: //preferred (minimum px); each wsnode generates 3 px: 1/1/1
            *px24ptr++ = ::WHITE; //ws start bit; only needs to be done 1x
            *px24ptr++ = px24 | Abits(::WHITE); //ws data bit
            *px24ptr++ = ::BLACK; //ws stop bit; only needs to be done 1x
...
class Pivot24: public OGLIO
{
    ...
    enum { B2b = 8}; //8 bits/byte
    enum { NUMCH = 24}; //fixed limit; uses all RGB bits
    enum { MAXCHLEN = 1M / 20 / 30}; //max #nodes @30 usec each slower frame rate (20 fps)
//    using col_t = ary<Pivot24, data_t>;
//    using row_t = ary<Pivot24, col_t, data_t>;
//    FBPixels m_shadowFB; //copy pixels to another screen/window
//    const size_t m_chqbytes; //#quadbytes/channel
...
//pad to avoid memory contention if using multiple CPUs:
    using data_t = uint32_t; //uint8_t; //use quad bytes to allow denser indexing
    enum { CHQBYTES = divup(MAXCHLEN * sizeof(data_t), CACHELEN) * CACHELEN / sizeof(data_t) };
...
//        size_t ixy = xyinx(y, x);
//help caller to debug indexing errors (assumes low bandwidth):
        if ((info.Length() < 2) || !info[0].IsNumber() || !info[1].IsNumber() || !inbounds(y, x) || ((info.Length() > 2) && !info[2].IsNumber())) return err_napi(info.Env(), "ch 0..%'d, ofs 0..%'d, optional bits (all Numbers) expected", NUMCH - 1, chbits() - 1);
        if (info.Length() > 2)
        {
            const auto bitval = info[2].ToNumber().Uint32Value(); //As<Napi::Number>().Uint32Value();
//debug("color 0x%x", color);
...
//        size_t ixy = xyinx(x, y);
//help caller to debug indexing errors (assumes low bandwidth):
        if ((info.Length() < 2) || !info[0].IsNumber() || !info[1].IsNumber() || !inbounds(y, x) || ((info.Length() > 2) && !info[2].IsNumber())) return err_napi(info.Env(), "ch 0..%'d, delay 0..%'d, optional bits (all Numbers) expected", NUMCH - 1, chqbytes() - 1);
        if (info.Length() > 2)
        {
            const auto bits = info[2].As<Napi::Number>().Uint32Value();
//debug("color 0x%x", color);
...
debug("fill(0x%x) %'lu qbytes took %'d usec (excl refresh)", bits, NUMCH * SIZEOF(m_chdata), now_usec() - started);
        dirty(true);
    }
#ifdef USING_NAPI
    Napi::Value fill_method(const Napi::CallbackInfo& info)
