...
//const makenew = require('./makenew');
const {debug, isobj} = require("./utils22");


module.exports = XPM;

//xpm loader ctor (data in memory):
//NOTE: origin [0,0] == top left
function XPM(data, name)
{
    if (!this instanceof XPM) return new XPM(data);
...
//    console.log("xpm got %d colors: %s".blue, numcolors, '{' + buf.substr(2) + '}');
    ...
    {
            if (data[ofs].length != w * chpp) throw "xpm bad row[" + y + "]: has len " + data[ofs].length + " (expected " + (w * chpp) + "): '" + data[ofs] + "'";
...
//console.log("img data len", /*img.data.length,*/ this.abgr.length, this.width, this.height);
    ...
    name = name? name + " ": "";
    console.log(name + "xpm loaded okay".cyan);
}

//xpm loader (from file):
//use glob so full path name not needed
XPM.fromFile = function(path)
{
    const path_fixup = path
        .replace(/^\~/, process.env.HOME)
        .replace(/^\.(?=\/)/, Pathlib.dirname(__stack[1].getFileName()));
    const files = glob.sync(path_fixup) || [];
    if (!files.length) throw `'${path_fixup}' not found`;
    if (files.length > 1) throw `'${path_fixup}' ambiguous: ${files.length} matches`;
    const image = fs.readFileSync(files[0]).toString()
        .replace(/^[^]*?=\s*\{\s*/, "") //trim leader
        .replace(/\s*\};\s*$/, ",") //trim trailer; look like other EOL
        .replace(/^"(.*?)",\s*$/gm, "$1") //remove quotes
        .split(/\n/); //convert to array of strings
//console.log(image[0], image[1], "...", image.slice(-1)); //replace(`(?<=[^]{200,}\\b)[^]*$`), " ...");
    const retval = new XPM(image, Pathlib.basename(path_fixup));
    [retval.name, retval.path] = [path, files[0]]; //use caller-recognized name but also give full path name
    return retval;
}

//check for XPM:
XPM.isXPM = function(obj)
{
    return isobj(obj) && obj instanceof XPM; //obj.constructor.name == "XPM"
}
...
//TODO? provide access to pixel data so caller can do custom draw:
//this.colors[this.colorinx[y][x]]; //actual color
Object.defineProperties(XPM.prototype,
{
...
debug("TODO: dim function, or arg to draw functions".brightYellow);


//change image size:
XPM.prototype.resize = function resize(w, h, newrgb)
{
//NOTE: use distinct arrays on each row to allow pixels to be set differently later
    ...
    while (this.height < h) this.colorinx.push(new Array(w).fill(newrgb || 0)); //grow, optional init new rows
    ...
    for (var y = 0; y < this.height; ++y)
    {
        ...
        if (wadjust > 0) this.colorinx[y].splice(w - wadjust, new Array(wadjust).fill(newrgb || 0)); //grow, optional init new cols
    }
...
XPM.prototype.hmirror_dup = function hmirror_dup()
{
    var old_width = this.width; //save; changes dynamically
    for (var y = 0; y < this.height; ++y)
    {
        var row = this.colorinx[y];
        for (var x = 0; x < old_width; ++x)
            row.push(row[old_width - x - 1]);
    }
//    this.width *= 2;
    this.img = null; //stale image data
    return this; //fluent
