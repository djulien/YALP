...
/*
this should be a palette, not an effect
async function color_wheel(model, opts)
{
    const {await_until} = model;
    const DURATION = opts.duration || 360 / (opts.fps || 5); //def 5 fps gives nice smooth, gradual fade
    const FPS = opts.fps || DURATION / 360, steplen = 1e3 / FPS, DURATION = opts.duration || 

    for (let hue = 0;; ++hue)
    {
        const color = rgb2RGB(hsv2rgb({h: hue % 360, s: 100, v: hsv_bradjust(hue % 360, 20)}));
