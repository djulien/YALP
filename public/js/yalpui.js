...
function reconnect(want_page)
{
    ...
    new SockJS(window.location.origin + '/reload') //'http://mydomain.com/my_prefix');
        ...
        {
            clearTimeout(retry); retry = null; //don't need to retry again
            console.log('opened');
            if (want_page !== false) window.location.reload(); //reload entire page now that server is back up
        }
        .onmessage = function onmsg(msg)
        {
            console.log('message', msg);
            if (msg.reload) loadjs(msg.script);
        }
        .onclose = function onclose() //server shut down or restarted
        {
//            sock = null;
            console.log('close');
