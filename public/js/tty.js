...
tty.open = function() {
//if (isNaN(++window.fido)) window.fido = 0;
//if (window.fido >= 0) fido();
...
console.log("elements", tty.elements, window.fido);
...
Tab.prototype.setProcessName = function(name) {
  name = sanitize(name);
console.log("set process name to '" + name + "'");
...
on(document, 'load', function() { load(1); });
on(document, 'DOMContentLoaded', function() { load(2); });
setTimeout(function() { load(3); }, 200);
