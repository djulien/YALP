...
var basicAuth = require('basic-auth-connect'); //not part of Express 4x -DJ
express.basicAuth = basicAuth; //http://stackoverflow.com/questions/23616371/basic-http-authentication-with-node-and-express-4
var serveStatic = require('serve-static') //not part of Express 4x -DJ
express.static = serveStatic; //backwards compatibility -DJ
...
//  this.io = io.listen(this.server, conf.io || {
  this.io = new io(this.server, conf.io || { //socket.io 1.x API -DJ
    log: false
  });
...
debugger;
  if (conf.static) {
console.log("serving static1 from ", conf.static);
    this.use(express.static(conf.static));
  }

  // If there is a custom favicon in the custom
  // static directory, this will be ignored.
//console.log("serving icon from ", __dirname + '/../static/favicon.ico');
//debugger;
//broken, and there doesn't seem to be a replacement -DJ
//not very useful with SPA anyway
//  this.use(express.favicon(__dirname + '/../static/favicon.ico'));

//  this.use(this.app.router); //deprecated in Express 4.x -DJ

//this should be else -DJ
  else {
console.log("serving static2 from ", __dirname + '/../public');
...
console.log("init io");
//  io.configure(function() { //deprecated in socket.io 1.x -DJ
//    io.disable('log');
//  });
...
console.log("tty: handle connection");
...
debugger;
  ...
  if (isNaN(++Session.prototype.handleCreate.num_cre)) Session.prototype.handleCreate.num_cre = 0; //show a counter -DJ
  this.log(
    'Created pty (id: %s, master: %d, pid: %d).',
    id, term.fd, term.pid, '#' + Session.prototype.handleCreate.num_cre);
...
Session.prototype.handleDisconnect = function() {
  ...
  if (isNaN(++Session.prototype.handleDisconnect.num_discon)) Session.prototype.handleDisconnect.num_discon = 0; //show a counter -DJ
  this.log('Client disconnected.', '#' + Session.prototype.handleDisconnect.num_discon);
