...
//        if (m_wrofs + len > m_buffer.byteLength) throw "Buffer overflow: " + m_wrofs + "+" + len;
...
///*static*/ void showbuf(const char* desc, const void* buf, int buflen, bool full) from RenXt_api.cpp 1.14
        ...
        for (var ofs = 0; ofs < buffer.byteLength; ++ofs)
        {
            debug(10, "%s (%d bytes):", desc, buffer.byteLength);
            for (var i = 0; i < buffer.byteLength; i += 16)
            {
                var linebuf; //char linebuf[sizeof(prevline)];
                var curlen = 0;
                for (var j = 0; (j < 16) && (i + j < buffer.byteLength); ++j)
                {
                    linebuf += sprintf((ptr[i + j] < 10)? "%d ": "x%.2x ", ptr[i + j]);
                    if (want_full) continue; //no repeat check
                    var repeated = Math.min(16, buffer.byteLength - i) - j - 1; //(i + j + 1 < buflen)? 0: buflen - i - j;
                    for (var k = repeated; k > 0; --k)
                        if (ptr[i + j + k] != ptr[i + j]) { repeated = 0; break; }
                    if (repeated) { linebuf += sprintf("...+%dx", repeated); break; }
                }
                if (!want_full && /*i && (i + 16 < buflen) &&*/(i + 16 < buffer.byteLength) && (linebuf != prevline)) continue; //don't show dup lines, except on last
                if (!want_full && i && (i + 16 < buffer.byteLength) && (ptr[i] == ptr[i - 1])) //check for last char repeated for entire line
                {
