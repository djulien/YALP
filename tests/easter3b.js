...
//var risen = xpm(He_Is_Risen48x16_xpm);
img.xy = xy_gdoor;
...
var bunny = xpm(Bunny_med_side_xpm);
bunny.xy = xy_gdoor;
var hops = [34,15, 33,14, 32,13, 31,13, 30,12, 28,12, 26,11, 24,11, 23,11, 21,11, 19,12, 18,13, 16,14, 15,14, 14,15];

function prep(img, val)
{
    global.gc();
...
{
    if (!color || !fade) return {r: 0, g: 0, b: 0};
	var r = (color >>> 16) & 0xff;
	var g = (color >>> 8) & 0xff;
	var b = (color >>> 0) & 0xff;
	fade /= 255;
...
function image(img, fade, atx, aty)
{
    if (!atx) atx = 0;
    if (!aty) aty = 0;
    if (arguments.length < 2) fade = 255; //full
