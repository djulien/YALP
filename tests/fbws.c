...
void draw()
{
	uint32_t colors[] = {RGSWAP(0xff0000), RGSWAP(0x00ff00), RGSWAP(0x0000ff), 0xffff00, RGSWAP(0xff00ff), RGSWAP(0x00ffff), 0xffffff};
...
for (int y = 0; y < 37; ++y)
	for (int b = 0; b < 24; ++b) //last bit is partially hidden by hsync
	{
if (!b) printf("node[%d]: 0x%x\n", y, colors[y % nel(colors)]);
		uint32_t bv = colors[y % nel(colors)] & (0x800000 >> b);
