...
function fxloop()
{
    const retval =
    {
        runlen: 60e3, //how long bkg loop runs
        looplen: 10e3, //how long (big) frbufs are; msec; == 200 fr @20 FPS
    };
...
const {HUE, PAL, hsv2RGB, /*HSV2RGB, HSV2hsv, RGB2rgb*/} = require("./color-mgmt.js");
...
//            const color = [PAL.RED_DIM, PAL.GREEN_DIM, PAL.BLUE_DIM][Math.floor(portinx / 8)];
//            const gap = portinx % 8 || 8; //use 8 in place of 0
//debug("here2", typeof frifo, (frifo.constructor || {}).name);
            wkstate.render_count_bump();
//            frifo.get_next(frnum, portinx); //propagate node values from prev frame
//            px.fill(dim(color, frtime / 2e3));
//            for (let node = 0; node < 300; node += port % 8 + 2)
//            for (let node = 0+1; node < anim_frnum+1; ++node)
//            px[anim_frnum + 1] = hsv2RGB(color); //hsvdim(color, frtime / 2e3));
            pin_finder(frbuf, portinx, anim_frnum, NPX);
        }
    }
...
function pin_finder(frbuf, portinx, anim_frnum, NPX)
{
...
//if (!frnum) debug(portinx, portname, color, hex(hsv2RGB(color || PAL.BLACK)), gap);
    ...
    for (let node = 0; node < 300; node += gap + 1)
         px[(NPX || 0) + node + anim_frnum % 8] = hsv2RGB(color); //pin finder
}
