...
const tree = /*new Tree*/({portnum: DPI24.R3});
tree.maxbr = 0.75
//const {Fence} = require("./models/fence");
const fence = /*new Fence*/({portnum: DPI24.R2});
//const {Devpanel} = require("./models/devpanel");
const devpanel = /*new Devpanel*/({portnum: -1});
const gift = {portnum: DPI24.B6};
gift.maxbr = 0.6;
fence.segs =
{
    RCandle: 5,
...
const {HUE, PAL, hsv2RGB, /*HSV2RGB, HSV2hsv, RGB2rgb*/} = require("./incl/color-mgmt.js");
...
//    for (let node = 0; node < 32 * (anim % 24); ++node) treepx[node + NPX1] = 0xFF00A000; //0xFF004000;
    treepx[0] = 0; //NPX
    for (let node = 0; node < 24 * 33; ++node) treepx[NPX1 + node] = 0x000600; //0xFF000800;
//const fence = pixels3D[frnum][DPI24.R2]; //.fill(hsv2RGB(PAL.GREEN_DIM)); //tree
    const giftpx = pixels3D[frnum][gift.portnum];
    for (let node = 0; node < 23 * 36; ++node) giftpx[node] = 0x000606; //0xFF000202;
...
//for (let node = 0; node < 2*50; ++node) fence[node] = hsv2RGB({H: ((node / 6) & 1)? HUE.BLUE: HUE.GREEN, S: 100, V: 33}); //, PAL.BLUE_DIM: PAL.GREEN_DIM);
    fencepx[0] = 0; //NPX
    Object.values(fence.segs).reduce((nodeofs, numpx, inx) =>
    {
