...
const {RED, GREEN, BLUE, CYAN, GOLD, BLACK, BLUE_dim, WHITE_dim, WARM_WHITE, COOL_WHITE, RGBdim} = require("../incl/colors");
...
const {devpanel, ic, globes, mtree} = models;
...
//profile info: [#ch, chname re]
    const profile =
    {
        ...
        cross: /^Cross/,
        mtree: [12*2, /^Mtree 1A/],
        mtbank: [2, /^Mtree off=GR\/ON=BW BankA/], //{BW BankA, RW BankA, BW BankB, RW BankB}
...
//    spare: /^spare 92/, //{92, 93, 94, blank, dead} some spares used?!
        acc: [5, /^FPArch 1/], //{fparch/instr/sidewk/heart 1, 2, 3, 4, 5} instr2+3  = drum sticks
        accbank: [2, /^Acc Bank off=01/], //{sel 23, sel 13} FPArch 1/Instr 2/Sidewalk 3/Heart 4
        shep: [4, /^Shep 1\/Guitar/], //{Shep 1 Guitar, Shep 2 Drums, Shep 3 Oboe, Shep 4 Sax}
...
//            [name + "_numch"]: info[0],
        ...
        .reduce((obj, [name, info]) => Object.assign(obj, {[name]: info}), {});
debug(retval);
//verify all data for a prop is contiguous:
    assert(retval.mtbank.chinx == retval.mtree.chinx + 2*12, `${retval.mtbank} ${retval.mtree}`);
    assert(retval.trballs.chinx == retval.mtree.chinx + 2*12 + 4);
    assert(retval.acc.chinx == retval.accbank.chinx + 2);
    assert(retval.shbank.chinx == retval.sheep.chinx + 6);
...
{
//return;
    const {ic_chinx, ab_chinx, af_chinx} = vix2ch(findch, getchval);
    const icw = Math.round(ic.width / 14); //, icrem = ic.width - 13 * icw;
...
if (!(prevfr % 300)) debug("house output+pre-rend fr# %'d/timest %'d, wait ctlr next, mp3 time %'d sec", prevfr, prevtime, mp3play.timestamp || 0);
...
//big props:
//ic x14
//for now, map each channel to 1/14th of real ic (~11x10)
        const CYAN_ic = RGBdim(CYAN, 1/2); //can be bright since diffused
        const ic_chvals = getchval(ic_chinx, frinx, 14);
        for (let i = 0; i < ic_chvals.length; ++i)
            ic.nodes1D.fill(RGBdim(CYAN_ic, ic_chvals[i] / 255), ic.height * i * icw, ic.height * (i + 1)  * icw);
//archfans: x64 {a 1, a 2, a 3, a 4, f 1, f 2, f 3, f 4} 8 each
//archfans are more expressive, so they should override angelbells
TODO("archfans -> fence?");
        const WHITE_arch = RGBdim(WARM_WHITE, 0.6), WHITE_fan = RGBdim(COOL_WHITE, 0.6);
...
TODO("add propsel ch for ab vs. af to reduce memory access at runtime");
        if (want_archfans)
            for (let af = 0; af < 4; ++af)
            {
                ...
                for (let a = 0; a < 8; ++a)
                    Arch2Globe[a].forEach(({x, y}) => globes[afrot].nodes2D[x][y] = RGBdim(WHITE_arch, af_chvals[16 * af + a] / 255));
                for (let f = 0; f < 8; ++f)
                    Fan2Globe[f].forEach(({x, y}) => globes[afrot].nodes2D[x][y] = RGBdim(WHITE_arch, af_chvals[16 * af + 8 + f] / 255));
            }
        else
        {
...
//        const GOLD = 0xFF404000, RED = 0xFF400000, GREEN = 0xFF004000, BLUE = 0xFF000040;
            ...
            for (let ab = 0; ab < 8; ++ab)
            {
                ...
                if (ab_chvals[bell]) glcolor[abrot] = RGBdim(ab_pal[ab % ab_pal.length], ab_chvals[bell]);
                else if (ab_chvals[hdwings]) glcolor[abrot] = RGBdim(GOLD_ab, ab_chvals[hdwings]);
                else if (ab_chvals[body]) glcolor[abrot] = RGBdim(GOLD_ab, ab_chvals[body]);
            }
...
//each channel mapped to column of real mtree
        ...
        const GREEN_mtree = RGBdim(GREEN, 0.7), BLUE_mtree = RGBdim(BLUE, 0.7), RED_mtree = RGBdim(RED, 0.7), WHITE_mtree = RGBdim(WHITE, 0.5);
        ...
        for (let x = 0; x < 2*12; ++x)
            mtree.nodes2D[flip(x, 2*12)].fill(RGBdim((x & 1)? mtree_colorA: mtree_colorB, mtree_chvals[x]));
//TODO: tree balls

//nativity figures/props:
//angel: {body, wings, trumpet}
        const angel_chvals = getchval(angel_chinx, frinx, 3);
        const GOLD_angel = RGBdim(GOLD, 0.5), WHITE_angel = RGBdim(WARM_WHITE, 0.5);
        const angel_bcolor = RGBdim(WHITE_angel, angel_chvals[0] / 255);
        const angel_hcolor = RGBdim(GOLD_angel, angel_chvals[0] / 255); //hair+body
        const angel_wcolor = RGBdim(GOLD_angel, angel_chvals[1] / 255);
        const angel_trcolor = RGBdim(GOLD_angel, angel_chvals[2] / 255); //halo+trumpet
...
//star: {b aura, y inner, w outer}
        ...
        const BLUE_star = RGBdim(BLUE, 0.5), WHITE_star = RGBdim(WARM_WHITE, 0.5), YELLOW_star = RGBdim(YELLOW, 0.5);
        const star_acolor = RGBdim(BLUE_star, star_chvals[0] / 255);
        const star_icolor = RGBdim(YELLOW_star, star_chvals[1] / 255);
        const star_ocolor = RGBdim(WHITE_star, star_chvals[2] / 255);
...
//MJB: {M, J, B, stable}
        ...
        const WHITE_mjb = RGBdim(WARM_WHITE, 0.4), BROWN_stable = RGBdim(ORANGE, 0.4);
        const stable_color = RGBdim(BROWN_stable, MJB_chvals[3] / 255);
        const gift_corners = { 0: 0, 1: 0, 2: -4, 3: -3, /*...,*/ [-4]: -3, [-3]: -4, [-2]: 0, [-1]: 0, };
        mary.fill(RGBdim(WHITE_mjb, MJB_chvals[0]));
        joseph.fill(RGBdim(WHITE_mjb, MJB_chvals[1]));
        Baby.fill(RGBdim(WHITE_mjb, MJB_chvals[2]));
...
//wisemen:
        ...
        const CYAN_wm = RGBdim(CYAN, 0.5), PINK_wm = RGBdim(MAGENTA, 0.5), GOLD_wm = RGBdim(GOLD, 0.5), WHITE_wm = RGBdim(WARM_WHITE, 0.3), GREEN_wm = RGBdim(GREEN, 0.5);
        wisemen[0].fill(RGBdim(CYAN_wm, wm_chvals[0] / 255));
        wisemen[1].fill(RGBdim(PINK_wm, wm_chvals[1] / 255));
        wisemen[2].fill(RGBdim(GREEN_wm, wm_chvals[2] / 255));
        for (let i = 0; i < 3; ++i) wisemen[i].fill(RGBdim(GOLD_wm, wm_chvals[0] / 255), wisemen[i].head);

//shep:
        const shep_chvals = getchval(shep_chinx, frinx, 4);
        const GOLD_shep = RGBdim(GOLD, 0.5), WHITE_shep = RGBdim(WARM_WHITE, 0.3);
        for (let i = 0; i < 4; ++i) shep[i].fill(RGBdim(WHITE_shep, shep_chvals[i] / 255));
...
//tree: x24 + 4 banksel {BW BankA, RW BankA, BW BankB, RW BankB}
        ...
        for (let x = 0, xofs = 0, yofs = 0; x < 2*12; ++x)
            this.nodes2D[xofs + x][yofs + 0] = this.nodes2D[xofs + x][yofs + 1] = RGBdim((x & 1)? mtree_colorA: mtree_colorB, mtree_chvals[x]);
